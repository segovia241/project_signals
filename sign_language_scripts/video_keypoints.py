...
class VideoSignLanguagePredictor:
    ...
    def initialize_model(self):
        ...
        try:
            ...
            self.drawing_spec_hands = self.mp_drawing.DrawingSpec(thickness=2, circle_radius=2, color=(0, 255, 255))
            self.drawing_spec_face = self.mp_drawing.DrawingSpec(thickness=1, circle_radius=1, color=(255, 255, 0))
    ...
    def create_widgets(self):
        ...
        # Checkbox para mostrar/ocultar keypoints
        self.show_keypoints_var = tk.BooleanVar(value=True)
        self.keypoints_checkbox = ttk.Checkbutton(
            button_frame, 
            text="Mostrar Keypoints", 
            variable=self.show_keypoints_var
        )
        self.keypoints_checkbox.pack(side=tk.LEFT, padx=(20, 0))
        ...
        ttk.Label(results_frame, text="INFORMACIÓN DE DETECCIÓN:", 
                 font=('Arial', 10, 'bold')).grid(row=5, column=0, sticky=tk.W)
        
        self.detection_info_label = ttk.Label(results_frame, text="Manos: No detectadas\nPose: No detectada", 
                                            font=('Arial', 9))
        self.detection_info_label.grid(row=6, column=0, sticky=tk.W, pady=(0, 10))
        
        # Todas las predicciones
        ttk.Label(results_frame, text="TODAS LAS PREDICCIONES:", 
                 font=('Arial', 10, 'bold')).grid(row=7, column=0, sticky=tk.W)
        
        self.predictions_text = tk.Text(results_frame, height=8, width=30, 
                                      font=('Arial', 9))
        self.predictions_text.grid(row=8, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        ...
        content_frame.columnconfigure(0, weight=1)
        content_frame.columnconfigure(1, weight=1)
        results_frame.rowconfigure(8, weight=1)
        results_frame.columnconfigure(0, weight=1)
    ...
    def stop_video(self):
        ...
        self.detection_info_label.config(text="Manos: No detectadas\nPose: No detectada")
    ...
    def analyze_video(self):
        ...
        def analysis_thread():
            ...
            while self.is_playing and self.cap.isOpened():
                ...
                # Procesar frame y dibujar keypoints
                processed_frame, keypoints, has_hand_detection, results = self.process_frame_with_keypoints(frame)
                ...
                # Mostrar frame procesado en la interfaz
    ...
    def process_frame_with_keypoints(self, frame):
        ...
        # Extraer keypoints para el modelo
        keypoints, has_hand_detection, _ = self.extract_keypoints(frame)
        
        # Actualizar información de detección
        detection_info = self.get_detection_info(results)
        self.root.after(0, lambda: self.detection_info_label.config(text=detection_info))
        
        return annotated_frame, keypoints, has_hand_detection, results
    ...
    def draw_landmarks(self, image, results):
        ...
        # Dibujar puntos faciales seleccionados
    ...
    def get_detection_info(self, results):
        """Obtener información de qué elementos se detectaron"""
        info = []
        
        # Información de manos
        left_hand = "✓" if results.left_hand_landmarks else "✗"
        right_hand = "✓" if results.right_hand_landmarks else "✗"
        info.append(f"Manos: Izq[{left_hand}] Der[{right_hand}]")
        
        # Información de pose
        pose = "✓" if results.pose_landmarks else "✗"
        info.append(f"Pose: [{pose}]")
        
        # Información de cara
        face = "✓" if results.face_landmarks else "✗"
        info.append(f"Cara: [{face}]")
